...
void ofApp::setup(){
    ...
    // Pista de sonido con la locución que se repite
...
void ofApp::update(){
    ...
    // Actualiza el sonido de la locución
    updateSound(false);
...
void ofApp::keyPressed(int key){
    ...
    if ((key == 'a') or (key == 'A') or (key == '6'))
    {
        ...
        // Indica que se ha pulado el botón
        updateSound(true);
    }
...
void ofApp::updateSound(bool buttonPusshed){
    ...
    // estadoSonido = 0 se reproduce la locución larga
    // estadoSonido = 1 suenan los pitidos
    // estadoSonido = 2 tiempo de visión termográfica
    
    // tiempoAtenua1  = primera reducción sonido (seg)
    // volumenAtenua1 = primer volumen atenuado
    // tiempoAtenua2  = segundo reducción sonido (seg)
    ...
    if (estadoSonido==0)
    {
        ...
        // Gestión de la pulsación del boton
        if (buttonPusshed)
        {
            estadoSonido = 1;
            elLoro1.stop();
